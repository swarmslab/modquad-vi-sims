...
acc_y_list = []
acc_z_list = []
center_of_struct_x_list = []
center_of_struct_y_list = []
center_of_struct_z_list = []
imu_time_list = []
...
fig2 = plt.figure(2,figsize=(length,width))
...
figname = '/home/guanrui/my_paper/Vi-ModQuad-ICRA-2019/figs/Struct-traj.png'
...
#fig1.set_ylabel(r'$r_{S_x}^{W}\hspace{0.5} (m)$',fontsize = label_font)
...
for odom1, odom2 in zip(modquad1_vicon, modquad2_vicon):
    center_of_struct_x_list.append((odom1.pose.pose.position.x+odom2.pose.pose.position.x)/2) 
    center_of_struct_y_list.append((odom1.pose.pose.position.y+odom2.pose.pose.position.y)/2) 
    center_of_struct_z_list.append((odom1.pose.pose.position.z+odom2.pose.pose.position.z)/2) 
...
struct_pos_x = np.asarray(center_of_struct_x_list)
struct_pos_y = np.asarray(center_of_struct_y_list)
struct_pos_z = np.asarray(center_of_struct_z_list)
...
trajx = np.asarray([1.0,1.0,1.0,2.2,2.2,1.0,1.0,1.0,1.0]) 
trajy = np.asarray([0.5,0.5,0.5,0.5,1.5,1.5,0.5,0.5,0.5])
trajz = np.asarray([0.0,0.5,1.3,2.0,1.3,2.0,1.3,0.5,0.0])
ax.plot(struct_pos_x, struct_pos_y, struct_pos_z, label='parametric curve')
ax.plot(trajx, trajy, trajz,'r', label='parametric curve')
#ax.plot(struct_pos_x, struct_pos_y,'r', label='x-y')
#ax.plot(struct_pos_y, struct_pos_z,zdir = 'x','g', label='y-z')
#ax.plot(struct_pos_x, struct_pos_z,'b', label='x-z')
