...
for bag_name in bag_lists:
	...
	vicon_positionz_list = []
        hor_lin1 = []
        hor_lin2 = []
        vision_position_sim_x_list = []
        vision_position_sim_y_list = []
        vision_position_sim_z_list = []
        vision_velocity_sim_x_list = []
        vision_velocity_sim_y_list = []
        vision_velocity_sim_z_list = []
	imu_ang_x_list = []
	imu_ang_y_list = []
	imu_ang_z_list = []
	imu_acc_x_list = []
	...
	print "whycon position error in z %s " %np.std(whycon_position_z_list)
        ...
        print "acceleration bias error in x %s "%np.std(imu_acc_x_list)
        print "acceleration bias error in y %s "%np.std(imu_acc_y_list)
        print "acceleration bias error in z %s "%np.std(np.asarray(imu_acc_z_list)-9.8)


	for topic, position, stamp in bag.read_messages(topics=['/modquad1/filtered_Vision_Odom_sim']):
	    time = stamp.secs + stamp.nsecs * 1e-9
	    vision_position_sim_x_list.append(position.position.x)
	    vision_position_sim_y_list.append(position.position.y)
	    vision_position_sim_z_list.append(position.position.z)
	    vision_velocity_sim_x_list.append(position.velocity.x)
	    vision_velocity_sim_y_list.append(position.velocity.y)
	    vision_velocity_sim_z_list.append(position.velocity.z)
	    simrostime_list.append(time)

	simrostime = np.asarray(simrostime_list)
	simrostime = simrostime - simrostime_list[0] 

	start_time=rostime[0]
	end_time=rostime[-1]

	vision_position_sim_x = np.asarray(vision_position_sim_x_list)
	vision_position_sim_y = np.asarray(vision_position_sim_y_list)
	vision_position_sim_z = np.asarray(vision_position_sim_z_list)
	vision_velocity_sim_x = np.asarray(vision_velocity_sim_x_list)
	vision_velocity_sim_y = np.asarray(vision_velocity_sim_y_list)
	vision_velocity_sim_z = np.asarray(vision_velocity_sim_z_list)
	...
	ax223.set_xlim(start_time, end_time)
        ...
        pdb.set_trace()
	ax121.plot(simrostime, vision_position_sim_x, 'r'+line[i],label = 'sim')
	ax122.plot(simrostime, vision_position_sim_y, 'r'+line[i],label = 'sim')
	ax123.plot(simrostime, vision_position_sim_z, 'r'+line[i],label = 'sim')

	ax221.plot(simrostime, vision_velocity_sim_x, 'r'+line[i],label = 'sim')
	ax222.plot(simrostime, vision_velocity_sim_y, 'r'+line[i],label = 'sim')
	ax223.plot(simrostime, vision_velocity_sim_z, 'r'+line[i],label = 'sim')
